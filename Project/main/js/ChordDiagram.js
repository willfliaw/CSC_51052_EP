...
function generateCountryColors() {
    ...
    Object.entries(ctx.CONTINENT_TO_COUNTRIES).forEach(([continent, countries]) => {
...
function loadData() {
    ...
    d3.csv("data/ChordDiagram_Dataset.csv").then(function (data) {
        
        ctx.refugeeTeam = "Refugee Olympic Team";

      
        ctx.CONTINENTS = [...new Set(data.map(d => d.birth_continent_name))];
        ...
        data.forEach(d => {
            ...
            if (!countryCounts[birthCountry]) countryCounts[birthCountry] = { in: 0, out: 0 };
            if (!countryCounts[countryName]) countryCounts[countryName] = { in: 0, out: 0 };
            countryCounts[birthCountry].out++;
            countryCounts[countryName].in++;
        });

      
        const validCountries = Object.keys(countryCounts).filter(country =>
            countryCounts[country].in > 50 || countryCounts[country].out > 10
        );
        ...
        const filteredData = data.filter(
            d => validCountries.includes(d.birth_country) && validCountries.includes(d.country_name)
        );
        console.log("data ",data)
        console.log("filteredData ",filteredData)

        ctx.CONTINENT_TO_COUNTRIES = ctx.CONTINENTS.reduce((acc, continent) => {
...
function createDirectedChordDiagram(matrix, countryCounts) {
    const svg = d3.select("#main").append("svg")
        ...
        .append("g")
        .attr("transform", `translate(${ctx.CHART_WIDTH / 2}, ${ctx.CHART_HEIGHT / 2})`);
    ...
    group.append("path")
       // .style("fill", d => {
        ...
        .style("stroke", d => d3.rgb(ctx.COUNTRY_COLORS[ctx.COUNTRIES[d.index]]).darker())
        ...
        .on("mouseover", (event, d) => {
            ...
            svg.selectAll(".ribbon")
                .style("opacity", 0.08);
            svg.selectAll(`.ribbon.source-${d.index}, .ribbon.target-${d.index}`)
                .style("opacity", 1);
    ...
    svg.append("g")
        ...
        .data(chord)
        .enter().append("path")
        .attr("class", d => `ribbon source-${d.source.index} target-${d.target.index}`)
        .attr("d", ribbon)
       // .style("fill", d => {
        //    const continent = Object.keys(ctx.CONTINENT_TO_COUNTRIES).find(continent =>
      ...
      //      );
       ...
       .style("stroke", d => d3.rgb(ctx.COUNTRY_COLORS[ctx.COUNTRIES[d.target.index]]).darker())
        .style("opacity", 0.7);
    ...
    continentGroups.forEach(([continent, countries]) => {
        ...
        console.log(`Continent: ${continent}, StartAngle: ${startAngle}, EndAngle: ${endAngle}`);
        const colorScale = continentColorFamilies[continent];
